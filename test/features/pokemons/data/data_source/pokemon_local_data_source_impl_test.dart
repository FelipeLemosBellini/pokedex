  ...
  group('PokemonLocalDataSourceImpl', () {
    group('cachePokemons', () {
      ...
      test('deve retornar Failure quando ocorrer erro ao salvar', () async {
        ...
        result.fold((success) => fail('Não deveria retornar sucesso'), (error) {
          expect(error, isA<Exception>());
          expect(error.toString(), contains('Erro ao salvar'));
        });

        verify(
          () => mockLocalStorage.setString('POKEMONS_CACHE', any()),
        ).called(1);
      });
    ...
    group('getCachedPokemons', () {
      test(
        'deve retornar Success com lista de pokemons quando há cache',
        () async {
      ...
      test('deve retornar Failure quando não há cache (null)', () async {
        ...
        result.fold((success) => fail('Não deveria retornar sucesso'), (error) {
          ...
          expect(error.message, "cache not found");
        });
      ...
      test(
        'deve retornar Failure quando a estrutura do JSON está errada',
        () async {
          ...
          expect(result.isError(), true);
          result.fold((success) => fail('Não deveria retornar sucesso'), (
            error,
          ) {
            expect(error, isA<Exception>());
          });
      ...
      test(
        'deve retornar Success com lista vazia quando o cache está vazio',
        () async {
          const emptyListJson = '[]';
    ...
    group('hasCache', () {
      test('deve retornar true quando há cache', () async {
        when(
          () => mockLocalStorage.containsKey('POKEMONS_CACHE'),
        ).thenAnswer((_) async => true);
        ...
        result.fold(
          (onSuccess) {
            expect(onSuccess, true);
          },
          (onFailure) {
            fail("Não deveria retornar erro");
          },
        );
